...
public class TodoDatabase {
  ...
  public TodoDatabase(String userDataFile) throws IOException {
    ...
    allUsers = objectMapper.readValue(reader, Todo[].class);
  }
   ...
   */
  public Todo[] listUsers(Map<String, List<String>> queryParams) {
    Todo[] filteredUsers = allUsers;
   ...
   *         age
   */
  public Todo[] filterUsersByAge(Todo[] users, int targetAge) {
    return Arrays.stream(users).filter(x -> x.age == targetAge).toArray(Todo[]::new);
  }
   ...
   *         company
   */
  public Todo[] filterUsersByCompany(Todo[] users, String targetCompany) {
    return Arrays.stream(users).filter(x -> x.company.equals(targetCompany)).toArray(Todo[]::new);
  }
